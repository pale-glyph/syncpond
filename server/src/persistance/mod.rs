...
impl PersistenceManager {
    ...
    pub fn new(base_path: PathBuf) -> Self {
        Self {
            ...
            bucket_cache: Arc::new(Mutex::new(LruCache::new(
                NonZeroUsize::new(BUCKET_CACHE_CAPACITY).unwrap(),
            ))),
        }
    }
    ...
    pub fn open_room_db(&self, room_id: u64) -> Result<(), String> {
        {
            let map = self
                .dbs
                .lock()
                .map_err(|_| "persistence mutex poisoned".to_string())?;
        ...
        let db =
            rocksdb::DB::open(&opts, &path).map_err(|e| format!("rocksdb open failed: {}", e))?;
        let mut map = self
            .dbs
            .lock()
            .map_err(|_| "persistence mutex poisoned".to_string())?;
    ...
    pub fn open_existing_room_dbs(&self) -> Result<(), String> {
        let entries =
            std::fs::read_dir(&self.base_path).map_err(|e| format!("read_dir failed: {}", e))?;
...
impl RoomPersistence {
    ...
    pub fn persist_fragment(
        ...
        if let Some(v) = value {
            ...
            self.db
                .put(key_str.as_bytes(), bytes)
                .map_err(|e| format!("rocksdb put failed: {}", e))
        } else {
            self.db
                .delete(key_str.as_bytes())
    ...
    fn update_member_list(
        ...
        member_vec.sort();
        let bytes =
            serde_json::to_vec(&member_vec).map_err(|e| format!("serialize error: {}", e))?;
        self.db
            .put(self.member_list_key().as_bytes(), bytes)
            .map_err(|e| format!("rocksdb put failed: {}", e))
    }
    ...
    pub fn load_members(&self) -> Result<std::collections::HashSet<String>, String> {
        match self
            .db
            .get(self.member_list_key().as_bytes())
            .map_err(|e| format!("rocksdb get failed: {}", e))?
        {
            Some(v) => {
                let list: Vec<String> =
                    serde_json::from_slice(&v).map_err(|e| format!("deserialize error: {}", e))?;
    ...
    fn update_bucket_list(&self) -> Result<(), String> {
        ...
        self.db
            .put(self.bucket_list_key().as_bytes(), bytes)
            .map_err(|e| format!("rocksdb put failed: {}", e))
    }
    ...
    pub fn load_all_fragments(
        &self,
    ) -> Result<HashMap<u64, HashMap<String, serde_json::Value>>, String> {
...
mod tests {
    ...
    fn test_persist_bucket_updates_room_bucket_list_key() {
        ...
        let stored = room
            .db
            .get(room.bucket_list_key().as_bytes())
            .unwrap()
            .unwrap();
